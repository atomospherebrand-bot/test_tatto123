...
import {
  ...
  masterSchema,
  mastersTable,
  type BotAvailabilityDay,
  type BotMasterSummary,
  portfolioItemSchema,
  portfolioTable,
  serviceSchema,
  servicesTable,
...
function formatRuLabel(date: Date): string {
  const day = date.getDate().toString().padStart(2, "0");
  const month = RU_MONTHS_GENITIVE[date.getMonth()] ?? "";
  return `${day} ${month}`.trim();
}

function createMasterSummary(master: Master): BotMasterSummary {
  return botMasterSummarySchema.parse({
    id: master.id,
    name: master.name,
...
export class DatabaseStorage {
  ...
  private async getServiceById(id: string): Promise<Service | undefined> {
    const rows = await this.database.select().from(servicesTable).where(eq(servicesTable.id, id)).limit(1);
  ...
  private mapPortfolio(row: typeof portfolioTable.$inferSelect): PortfolioItem {
    return portfolioItemSchema.parse({
      ...
      mediaType: (row.mediaType ?? "image") as PortfolioItem["mediaType"],
  ...
  private async ensureClientHasNoActiveBookings(
    clientPhone: string | undefined,
    clientTelegram: string | undefined,
    ignoreBookingId?: string,
  ): Promise<void> {
    const phone = clientPhone ? normalizePhone(clientPhone) : "";
    const telegram = clientTelegram?.trim() ?? "";

    if (!phone && !telegram) return;

    const conditions = [ne(bookingsTable.status, "cancelled")] as any[];
    if (ignoreBookingId) {
      conditions.push(ne(bookingsTable.id, ignoreBookingId));
    }

    if (phone && telegram) {
      conditions.push(or(eq(bookingsTable.clientPhone, phone), eq(bookingsTable.clientTelegram, telegram)));
    } else if (phone) {
      conditions.push(eq(bookingsTable.clientPhone, phone));
    } else if (telegram) {
      conditions.push(eq(bookingsTable.clientTelegram, telegram));
    }
    ...
    const now = new Date();
    const hasActive = rows.some((row) => toDateTime(row.date, normalizeTimeValue(row.time)).getTime() >= now.getTime());

    if (hasActive) {
      throw new Error("У клиента уже есть активная запись. Сначала отмените текущую запись.");
    }
  }
  ...
  private async isSlotAvailable(
    ...
    return existing.every((booking) => {
      const slot = formatSlot(
        date,
        normalizeTimeValue(booking.time),
        booking.duration,
      );
  ...
  async createBooking(input: InsertBooking): Promise<Booking> {
    ...
    await this.ensureClientHasNoActiveBookings(payload.clientPhone, payload.clientTelegram ?? undefined);

    const normalizedPhone = normalizePhone(payload.clientPhone);
    const normalizedTelegram = payload.clientTelegram?.trim();
    ...
    await this.database.insert(bookingsTable).values({
      ...
      clientTelegram: normalizedTelegram ?? null,
  ...
  async updateBooking(id: string, input: Partial<InsertBooking>): Promise<Booking | undefined> {
    ...
    const nextPhone = input.clientPhone ?? current.clientPhone;
    const nextTelegram =
      input.clientTelegram !== undefined ? input.clientTelegram ?? undefined : optional(current.clientTelegram);

    await this.ensureClientHasNoActiveBookings(nextPhone, nextTelegram, id);

    const normalizedPhone = normalizePhone(nextPhone);
    const normalizedTelegram = nextTelegram?.trim();
  ...
  async addPortfolioItem(input: Omit<PortfolioItem, "id">): Promise<PortfolioItem> {
    ...
    return this.mapPortfolio(inserted ?? {
      id: item.id,
      url: item.url,
      title: item.title,
      masterId: item.masterId ?? null,
      ...
      mediaType: item.mediaType,
      createdAt: new Date(),
    } as typeof portfolioTable.$inferSelect);
  }
  ...
  async getAvailabilityCalendar(
    ...
    const masterSummaries = masters.map((master) => createMasterSummary(master));
    const base = startDate ? new Date(`${startDate}T00:00:00`) : new Date();
    base.setHours(0, 0, 0, 0);

    const availability: BotAvailabilityDay[] = [];
    ...
    for (let offset = 0; offset < days; offset++) {
      ...
      const slotMap = new Map<string, BotMasterSummary[]>();

      const slotsPerMaster = await Promise.all(
        masters.map((master) => this.getAvailableSlots(master.id, isoDate, service.duration)),
      );

      slotsPerMaster.forEach((slots, index) => {
        const summary = masterSummaries[index];
      ...
      const slots = Array.from(slotMap.entries())
        .map(([time, masterList]) => ({
          time,
          masters: masterList.slice().sort((a, b) => a.nickname.localeCompare(b.nickname, "ru")),
        }))
        .sort((a, b) => a.time.localeCompare(b.time));

      availability.push({
        date: isoDate,
        label: formatRuLabel(currentDate),
        weekday: RU_WEEKDAYS[weekdayIndex] ?? "",
        weekdayShort: RU_WEEKDAYS_SHORT[weekdayIndex] ?? "",
        slots,
      });
    }
  ...
  async getMastersForSlot(
    serviceId: string,
    date: string,
    time: string,
  ): Promise<BotMasterSummary[]> {
    await this.ensureReady();
    ...
    const masters = await this.listActiveMasters();
    const available: BotMasterSummary[] = [];

    for (const master of masters) {
      const slots = await this.getAvailableSlots(master.id, date, service.duration);
      if (slots.includes(time)) {
        available.push(createMasterSummary(master));
      }
    }

    return available.sort((a, b) => a.nickname.localeCompare(b.nickname, "ru"));
  }
  ...
  async listPortfolioByMaster(
    ...
    page = 1,
    pageSize = 6,
  ): Promise<{ items: PortfolioItem[]; total: number; page: number; pageSize: number; totalPages: number }> {
    await this.ensureReady();

    const normalizedPage = Math.max(page, 1);
    const normalizedPageSize = Math.min(Math.max(pageSize, 1), 48);
    ...
    const items = rows.map((row) => this.mapPortfolio(row));
  ...
  async getAvailableSlots(masterId: string, date: string, duration: number): Promise<string[]> {
    ...
    const existing = await this.database
      ...
      .from(bookingsTable)
      .where(
        and(eq(bookingsTable.masterId, masterId), eq(bookingsTable.date, date), ne(bookingsTable.status, "cancelled")),
      );
    ...
    for (let hour = openingHour; hour < closingHour; hour++) {
      for (let minute = 0; minute < 60; minute += stepMinutes) {
        ...
        const endHour = slotToInsert.end.getHours();
        const endMinute = slotToInsert.end.getMinutes();
        ...
        const available = existing.every((booking) => {
          const slot = formatSlot(date, normalizeTimeValue(booking.time), booking.duration);
