...
export async function runMigrations(db: NodePgDatabase<any>): Promise<void> {
  ...
  await db.execute(sql`ALTER TABLE portfolio_items ADD COLUMN IF NOT EXISTS master_id uuid REFERENCES masters(id) ON DELETE SET NULL;`);
  await db.execute(sql`ALTER TABLE portfolio_items ADD COLUMN IF NOT EXISTS style text;`);
  await db.execute(sql`ALTER TABLE portfolio_items ADD COLUMN IF NOT EXISTS media_type text NOT NULL DEFAULT 'image';`);
  await db.execute(sql`ALTER TABLE portfolio_items ADD COLUMN IF NOT EXISTS thumbnail text;`);
  await db.execute(sql`ALTER TABLE portfolio_items ADD COLUMN IF NOT EXISTS created_at timestamptz NOT NULL DEFAULT now();`);
  await db.execute(sql`
    ALTER TABLE portfolio_items
    ADD CONSTRAINT IF NOT EXISTS portfolio_items_media_type_check
